...
namespace OpenCredentialPublisher.PublishingService.Data
{
    public class PublishRequest
    {
        ...
        public PublishRequest(string requestId, string clientId, string requestIdentity, string filepath, string fileType, string pathway, bool pushAfterPublish = false, string appUri = null, string pushUri = null) : this()
        {
            this.ClientId = clientId;
            this.RequestId = requestId;
            this.RequestIdentity = requestIdentity;
            this.Files.Add(File.Create(filepath, fileType));
            this.PushAfterPublish = pushAfterPublish;
            this.PushUri = pushUri;
            this.AppUri = appUri;
            this.Pathway = pathway;
        ...
        public void Deconstruct(out string requestId, out string clientId, out string revocationListId)
        {
            requestId = this.RequestId;
            clientId = this.ClientId;
            revocationListId = this.RevocationList?.PublicId ?? String.Empty;
        }
    ...
    public static class PublishRequestExtensions
    {
        ...
        public static File GetFile(this PublishRequest request, string fileType)
        {
            return request?.Files.Where(f => f.FileType == fileType).OrderByDescending(f => f.CreateTimestamp).FirstOrDefault();
        }
        ...
        public static File GetClr2_0(this PublishRequest request)
        {
            return request?.Files.Where(f => f.FileType == ClrFileTypes.Clr2_0).OrderByDescending(f => f.CreateTimestamp).FirstOrDefault();
        }
