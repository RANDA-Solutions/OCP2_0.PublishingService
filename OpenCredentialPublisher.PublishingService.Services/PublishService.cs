...
namespace OpenCredentialPublisher.PublishingService.Services
{
    ...
    public class PublishService : IPublishService
    {
        ...
        public async Task<string> ProcessRequestAsync(string id, ClrCredential clr, string clientId, bool pushAfterPublish = false, string pushUri = null)
        {
            if (String.IsNullOrEmpty(id))
                id = Guid.NewGuid().ToString("d");

            string filename = ClrFilename(id);

            var contents = System.Text.Json.JsonSerializer.Serialize(clr, 
                new System.Text.Json.JsonSerializerOptions { DefaultIgnoreCondition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull, Encoder = JavaScriptEncoder.UnsafeRelaxedJsonEscaping });

            var filepath = await _store.StoreAsync(filename, contents);

            var request = new PublishRequest(id, clientId, id, filepath, ClrFileTypes.Clr2_0, Pathways.Publish2_0, pushAfterPublish, _appBaseUri, $"{pushUri}");
            ...
            await _mediator.Publish(new Publish2_0ProcessRequestCommand(id, pushAfterPublish, pushUri));

            _context.ClrPublishLogs.Add(new ClrPublishLog(request.ClientId, id, request.PublishState, "Request issued to workflow"));
        ...
        public async Task<PublishStatusResult> GetAsync(string requestId, string clientId, string accessKeyBaseUrl, string scope, string endpoint, string method)
        {
            ...
            {
                if (request.ProcessingState == PublishProcessingStates.Complete && request.PublishState == PublishStates.Complete)
                {
        ...
        public async Task<string> GetCredentialsAsync(string accessKey)
        {
            ...
            if (request.Pathway != Pathways.Publish1_0)
            {
                var clrFileName = request?.GetFile(ClrFileTypes.SignedClr2_0)?.FileName;

                if (clrFileName == null)
                    return null;

                return await _store.DownloadAsStringAsync(clrFileName);
            }
        ...
        public async Task SetRequestStateAsync(string accessKey, string state)
        {
            var request = await _context.PublishRequests
               .Where(r => r.AccessKeys.Any(k => k.Key == accessKey && !k.Expired))
               .FirstOrDefaultAsync();
