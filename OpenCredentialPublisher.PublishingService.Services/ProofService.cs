...
namespace OpenCredentialPublisher.PublishingService.Services
{
    public class ProofService
    {
        ...
        public ProofService()
        {
            ...
            _keyDid = new Regex("did:key:(?<key>[a-km-zA-HJ-NP-Z1-9]+)(?:#(?<fragment>[a-km-zA-HJ-NP-Z1-9]+))?");
            _isDid = new Regex("^did:");
            _isUrl = new Regex("^http[s]?:");
        }

        public async Task<Proof> CreateProof(string originalJson = null, string verificationMethod = null, byte[] privateKeyBytes = null, byte[] publicKeyBytes = null)
        {
            JObject document;
            ...
            var proof = new Proof
            {
                ...
                Nonce = WebEncoders.Base64UrlEncode(UTF8Encoding.UTF8.GetBytes(Guid.NewGuid().ToString())),
                Type = "Ed25519Signature2020",
                ProofPurpose = "assertionMethod"
            };

            var proofString = System.Text.Json.JsonSerializer.Serialize(proof, new JsonSerializerOptions {  DefaultIgnoreCondition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull, Encoder = JavaScriptEncoder.UnsafeRelaxedJsonEscaping });
            JToken proofToken;
        ...
        public async Task<string> CanonicalizeCredential(string json)
        {
            ...
            if (proof != null)
            {
                foreach (var item in proof)
                {
                    ...
                    var proofObject = JsonConvert.DeserializeObject<Proof>(item.ToString());
                    var publicKeyBytes = await GetPublicKeyAsync(document, proofObject);
        ...
        {
            var jsonldSignature = new JsonLinkedDataSignature();
            var proof = System.Text.Json.JsonSerializer.Deserialize<Proof>(proofJson.ToString());
            ...
            var publicKeyBytes = await GetPublicKeyAsync(document, proof);
        ...
        private string GetBody(string json, Proof proof)
        {
        ...
        private async Task<byte[]> GetPublicKeyAsync(JObject document, Proof proof)
        {
            ...
            if (_isDid.IsMatch(verificationMethod))
            {
                if (_keyDid.IsMatch(verificationMethod))
                {
                    ...
                    if (match.Success)
                    {
                        var keyString = match.Groups["key"].Value;
                        byte[] keyBytes;
                        if (match.Groups["fragment"].Success)
                        {
                            var fragment = match.Groups["fragment"].Value;
                            keyBytes = CryptoMethods.Base58DecodeString(fragment);
                        }
                        else
                        {
                            keyBytes = CryptoMethods.Base58DecodeString(keyString);
                        }

                        var length = keyBytes.Length - 32;
                        if (length > 0)
                        {
                            var multiCodec = keyBytes.Take(length).ToArray();
                            return keyBytes.Skip(length).ToArray();
                        }
                        return keyBytes;
                    }
                }
                ...
                {
                    var issuer = System.Text.Json.JsonSerializer.Deserialize<Profile>(document["issuer"].ToString());
                    if (issuer != null)
                    {
                        if (issuer.Id.StartsWith("did:web"))
                        {
                            DidWebResolver didWebResolver = new DidWebResolver(new HttpClient());
                            var didDocument = await didWebResolver.ResolveDidDocumentAsync(issuer.Id);
                            if (didDocument != null)
                            {
                                var didKey = didDocument.VerificationMethod.FirstOrDefault(x => x.Id == verificationMethod);
                                if (didKey != null)
                                {

                                    var decodedBytes = CryptoMethods.Base58DecodeString(didKey.PublicKeyMultibase);
                                    if (decodedBytes != null)
                                    {
                                        var length = decodedBytes.Length - 32;
                                        if (length > 0)
                                        {
                                            var multiCodec = decodedBytes.Take(length).ToArray();
                                            return decodedBytes.Skip(length).ToArray();
                                        }
                                        return decodedBytes;
                                    }
                                    else
                                    {
                                        throw new Exception("Unable to decode key");
                                    }
                                }
                            }
                        }
            ...
            else if (verificationMethod.Contains("#")) // contains a key fragment 
            {
                var parts = verificationMethod.Split("#");
                var keyFragment = parts[1];
                return CryptoMethods.Base58DecodeString(keyFragment);
            }
        ...
        private string GetSignature(Proof proof)
        {
